...
const TaskDetails = () => {
  ...
  useEffect(() => {
    const fetchTaskDetails = async () => {
      ...
      const tsk = proj.tasks.find((t: Task) => t.id === taskId);
      if (!tsk) return;
        ...
        <div className="p-5 rounded-md  border border-gray-300 dark:border-zinc-800  flex flex-col lg:h-[80vh]">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3">
            <textarea
              ...
              className="w-full dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-md p-2 text-sm text-gray-900 dark:text-zinc-200 resize-none focus:outline-none focus:ring-1 focus:ring-[#0b996f]/80"
              rows={3}
            />
            <button
              onClick={handleAddComment}
              className="bg-gradient-to-l from-[#0b996f] to-[#0b996f]/80 transition-colors text-white text-sm px-5 py-2 rounded "
            >
              Post
            </button>
          </div>
