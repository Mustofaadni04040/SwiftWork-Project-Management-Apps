...
const ProjectCalendar = ({ tasks }) => {
  ...
  const getTasksForDate = (date) =>
    tasks.filter((task: Task) => isSameDay(task.due_date, date));
          ...
          <div className="grid grid-cols-7 gap-2">
            {daysInMonth.map((day) => {
              ...
              const hasOverdue = dayTasks.some(
                (t: Task) => t.status !== "DONE" && isBefore(t.due_date, today)
              );
              ...
              return (
                <button
                  ...
                  className={`sm:h-14 rounded-md flex flex-col items-center justify-center text-sm
                                    ${
                                      isSelected
                                        ? "bg-green-200 text-green-900 dark:bg-[#0b996f]/90 dark:text-white"
                ...
                >
                  ...
                  {dayTasks.length > 0 && (
                    <span className="text-[10px] text-[#0b996f] dark:text-[#0b996f]/90">
