...
const CreateProjectDialog = ({ isDialogOpen, setIsDialogOpen }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 dark:bg-black/60 backdrop-blur flex items-center justify-center text-left z-50">
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 w-full max-w-lg max-h-[600px] overflow-y-auto no-scrollbar text-zinc-900 dark:text-zinc-200 relative">
        <button
            ...
            {formData.team_members.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2">
                {formData.team_members.map((email) => (
                  <div
                    key={email}
                    className="flex items-center gap-1 bg-green-200/50 dark:bg-green-500/20 text-[#0b996f] dark:text-[#0b996f]/50 px-2 py-1 rounded-md text-sm"
                  >
                    {email}
                    <button
                      type="button"
